...
export class OrdersService {
  ...
  private async loadOrderParts(
    ...
    if (parts.length !== uniquePartIds.length) {
      ...
      const missingIds = uniquePartIds.filter(
        (partId) => !foundIds.has(partId),
      );
      throw new BadRequestException(`零件 ID${missingIds.join('、')} 不存在`);
    }
  ...
  private normalizeDraftItem(
    ...
    const fallbackUnitPrice = partId
      ...
      : 0;
    const unitPrice = partId ? (explicitUnitPrice ?? fallbackUnitPrice) : null;
    const priceLabel =
  ...
  async createOrder(payload: CreateOrderRequest, createdById: number) {
    ...
    const responsibleUserId =
      this.normalizeOptionalId(payload.responsibleUserId) ?? createdById;
    ...
    const newOrder = await this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        ...
        const responsibleUser = await this.loadActiveResponsibleUser(
          tx,
          responsibleUserId,
        );
        ...
        return this.writeOrderWithTasks(
          tx,
          {
            customerId: customer.id,
  ...
  async createQuickOrder(
    payload: CreateQuickOrderRequest,
    createdById: number,
  ) {
    const { customerId, targetDate, items } = payload;
    const responsibleUserId =
      ...
      const result = await this.prisma.client.$transaction(
        async (tx: Prisma.TransactionClient) => {
          ...
          if (existingPartIds.length > 0) {
            const existingPartMap = await this.loadOrderParts(
              tx,
              existingPartIds,
            );
  ...
  async createDraft(
    ...
    const responsibleUserId = this.normalizeOptionalId(
              ...
              itemsInput.length > 0
                ? {
                    create: itemsInput.map((item) =>
                      this.normalizeDraftItem(item, partMap),
                    ),
                  }
                : undefined,
          },
  ...
  async updateDraft(
    ...
    return this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        ...
        if (
          !existing ||
          !this.canAccessDraft(existing.createdById, userId, role)
        ) {
          throw new NotFoundException(`订单草稿 ID: ${id} 不存在`);
        }
        ...
        if (
          payload.responsibleUserId !== undefined &&
          nextResponsibleUserId !== undefined
        ) {
          const responsibleUser = await tx.user.findUnique({
        ...
        if (payload.items !== undefined) {
          ...
          const unknownIds = incomingExistingIds.filter(
            (itemId) => !existingItemIdSet.has(itemId),
          );
          if (unknownIds.length > 0) {
            throw new BadRequestException(
              `草稿明细 ID 不存在：${unknownIds.join(', ')}`,
            );
          }
          ...
          if (idsToDelete.length > 0) {
            await tx.orderDraftItem.deleteMany({
              where: { draftId: id, id: { in: idsToDelete } },
            });
          }
          ...
          const itemsToUpdate = itemsInput
            ...
            .filter(
              (
                entry,
              ): entry is { item: (typeof itemsInput)[number]; id: number } =>
          ...
          const itemsToCreate = itemsInput.filter(
            (item) => this.normalizeOptionalId(item.id) === undefined,
          );
  ...
  async deleteDraft(id: number, userId: number, role: UserRoleType = 'ADMIN') {
    return this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        const existing = await tx.orderDraft.findUnique({ where: { id } });
        if (
  ...
  async submitDraft(
    ...
    return this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        ...
        const customerId = this.normalizeOptionalId(
        ...
        const completeItems = draft.items.map((item, index) => {
          const partId = this.normalizeOptionalId(item.partId ?? undefined);
          const orderedQty =
            typeof item.orderedQty === 'number' ? item.orderedQty : null;

          if (!partId || !orderedQty || orderedQty <= 0) {
            throw new BadRequestException(
              `请补全第 ${index + 1} 项零件明细后再提交`,
            );
          }
        ...
        const responsibleUserId =
          this.normalizeOptionalId(draft.responsibleUserId ?? undefined) ??
          userId;
        const responsibleUser = await this.loadActiveResponsibleUser(
          tx,
          responsibleUserId,
        ...
        const order = await this.writeOrderWithTasks(
          tx,
          {
            customerId: customer.id,
            customerName: customer.name,
  ...
  private async validateMergedOrderMembers(
    tx: Prisma.TransactionClient,
    orderIds: number[],
    ...
    if (uniqueOrderIds.length < 2) {
      throw new BadRequestException('至少选择两张订单');
    }
    ...
    if (orders.length !== uniqueOrderIds.length) {
      ...
      throw new BadRequestException(`订单 ${missingIds.join('、')} 不存在`);
    }

    const missingCustomer = orders.find((order) => !order.customerId);
    if (missingCustomer) {
      throw new BadRequestException(
        `订单 ${missingCustomer.id} 缺少客户主数据，不能参与合并`,
      );
    }

    const customerId = orders[0].customerId!;
    const otherCustomer = orders.find(
      (order) => order.customerId !== customerId,
    );
    if (otherCustomer) {
      throw new BadRequestException('只能合并同一客户的订单');
    }

    const occupiedOrder = orders.find(
      (order) =>
        order.mergedMembership &&
        order.mergedMembership.mergedOrderId !== currentMergedOrderId,
    );
    if (occupiedOrder) {
      throw new BadRequestException(
        `订单 ${occupiedOrder.id} 已属于其他合并订单`,
      );
    }

    return { customerId, orders, orderIds: uniqueOrderIds };
  }

  private mergedOrderDetailInclude() {
    return {
    ...
    } satisfies Prisma.MergedOrderInclude;
  }

  private mapMergedOrderDetail(
    mergedOrder: Prisma.MergedOrderGetPayload<{
      include: ReturnType<OrdersService['mergedOrderDetailInclude']>;
    }>,
  ): MergedOrderDetail {
    return {
      id: mergedOrder.id,
      mergedNo: mergedOrder.mergedNo,
      customerId: mergedOrder.customerId,
      customerName: mergedOrder.customer.name,
      remark: mergedOrder.remark,
      status: mergedOrder.status,
      createdAt: mergedOrder.createdAt.toISOString(),
      updatedAt: mergedOrder.updatedAt.toISOString(),
      createdBy: mergedOrder.createdBy,
      orders: mergedOrder.orders.map((member) => ({
        id: member.order.id,
        customerId: member.order.customerId,
        customerName: member.order.customerName,
        status: member.order.status,
        createdAt: member.order.createdAt.toISOString(),
        items: member.order.items,
      })),
    };
  }

  async createMergedOrder(
    payload: CreateMergedOrderRequest,
    createdById: number,
  ) {
    return this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        const { customerId, orderIds } = await this.validateMergedOrderMembers(
          tx,
          payload.orderIds,
        );
        const temporaryNo = `TMP-${randomUUID().replace(/-/g, '').slice(0, 24)}`;
        const mergedOrder = await tx.mergedOrder.create({
          data: {
            mergedNo: temporaryNo,
            customerId,
            createdById,
        ...
        const datePart = mergedOrder.createdAt
          .toISOString()
          .slice(0, 10)
          .replace(/-/g, '');
        return tx.mergedOrder
          .update({
            where: { id: mergedOrder.id },
            data: {
              mergedNo: `MRG-${datePart}-${String(mergedOrder.id).padStart(6, '0')}`,
            },
            include: this.mergedOrderDetailInclude(),
          })
          .then((item) => this.mapMergedOrderDetail(item));
      },
    );
  }
  ...
  async listMergedOrders(
    ...
    pageSize: number = 20,
    keyword?: string,
  ): Promise<PaginatedMergedOrders> {
    const safePage = Math.max(1, Number(page) || 1);
    const safePageSize = Math.min(100, Math.max(1, Number(pageSize) || 20));
    const trimmedKeyword = keyword?.trim();
    const where: Prisma.MergedOrderWhereInput = {
      status: 'ACTIVE',
      ...(trimmedKeyword
        ? {
            OR: [
              { mergedNo: { contains: trimmedKeyword, mode: 'insensitive' } },
              {
                customer: {
    ...
    const [total, records] = await this.prisma.client.$transaction([
      this.prisma.client.mergedOrder.count({ where }),
      this.prisma.client.mergedOrder.findMany({
        where,
    ...
    return {
      ...
      data: records.map((record) => {
        const dates = record.orders.map((member) => member.order.createdAt);
        return {
          id: record.id,
          mergedNo: record.mergedNo,
          ...
          orderDateStart: dates.length
            ? new Date(
                Math.min(...dates.map((date) => date.getTime())),
              ).toISOString()
            : null,
          orderDateEnd: dates.length
            ? new Date(
  ...
  async getMergedOrder(id: number): Promise<MergedOrderDetail> {
    const mergedOrder = await this.prisma.client.mergedOrder.findFirst({
      where: { id, status: 'ACTIVE' },
      include: this.mergedOrderDetailInclude(),
    });
    if (!mergedOrder) {
      throw new NotFoundException(`合并订单 ID: ${id} 不存在`);
    }
    return this.mapMergedOrderDetail(mergedOrder);
  }

  async updateMergedOrder(id: number, payload: UpdateMergedOrderRequest) {
    return this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        const existing = await tx.mergedOrder.findFirst({
          where: { id, status: 'ACTIVE' },
        ...
        const requestedOrderIds =
          payload.orderIds ?? existing.orders.map((member) => member.orderId);
        const { customerId, orderIds } = await this.validateMergedOrderMembers(
          tx,
          requestedOrderIds,
          id,
        ...
        if (payload.orderIds) {
          await tx.mergedOrderItem.deleteMany({ where: { mergedOrderId: id } });
          await tx.mergedOrderItem.createMany({
            data: orderIds.map((orderId) => ({ mergedOrderId: id, orderId })),
          });
        }

        const updated = await tx.mergedOrder.update({
          where: { id },
          data: {
            customerId,
            ...(payload.remark !== undefined
        ...
        return this.mapMergedOrderDetail(updated);
      },
    );
  }

  async dissolveMergedOrder(id: number) {
    return this.prisma.client.$transaction(
      async (tx: Prisma.TransactionClient) => {
        const existing = await tx.mergedOrder.findFirst({
          where: { id, status: 'ACTIVE' },
  ...
  async findAll(
      ...
      this.prisma.client.order.findMany({
        ...
        include: {
          ...
          mergedMembership: {
            select: {
              mergedOrderId: true,
              mergedOrder: { select: { mergedNo: true } },
            },
          },
        },
      }),
    ...
    const data = rawData.map((order) => {
      ...
      const { mergedMembership, ...orderData } = order;
      return {
        ...orderData,
        items,
        totalAmount,
        mergedOrder: mergedMembership
          ? {
              id: mergedMembership.mergedOrderId,
              mergedNo: mergedMembership.mergedOrder.mergedNo,
            }
          : null,
      };
    });
    ...
    if (order.items) {
      for (const item of order.items) {
        if (item.productionTask) {
          (item.productionTask as any).urgency =
            ProductionTaskService.calculateUrgency(
              item.productionTask.targetDate,
            );
        }
      }
    }
