...
export function PartDrawingPreview({
  ...
  if (drawing) {
    const fileUrl = buildPartDrawingFileUrl(drawing.partId, drawing.id)
    const downloadUrl = buildPartDrawingFileUrl(drawing.partId, drawing.id, {
      download: true,
    })
      ...
      <PreviewShell errorMessage={errorMessage}>
        <div className='relative w-full aspect-[4/3] overflow-hidden rounded-xl border border-border bg-muted/30 sm:aspect-square'>
          {isImage ? (
            <button
              type='button'
              onClick={() => onImagePreview(fileUrl, drawing.fileName)}
