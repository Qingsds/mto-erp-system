...
const MOCK: Part[] = [
  { id:1, partNumber:"P-0041", name:"六角螺栓 M8×30",    material:"304不锈钢", spec:"GB/T 5782", remark:"标准紧固件", prices:[{label:"标准价",value:0.85},{label:"批量价",value:0.72}], createdAt:"2026-03-01" },
  { id:2, partNumber:"P-0040", name:"轴承座 UCF205",     material:"铸铁",     spec:"UCF205",    remark:"",          prices:[{label:"标准价",value:68}],                                  createdAt:"2026-02-28" },
  { id:3, partNumber:"P-0039", name:"密封圈 O-Ring 50×3",material:"丁腈橡胶", spec:"Φ50×3mm",   remark:"",          prices:[{label:"标准价",value:2.4},{label:"批量价",value:1.9}],       createdAt:"2026-02-20" },
  { id:4, partNumber:"P-0038", name:"精密导轨 HGR15-500",material:"GCr15钢",  spec:"L=500mm",   remark:"精密级",    prices:[{label:"标准价",value:320},{label:"批量价",value:280}],        createdAt:"2026-02-15" },
  { id:5, partNumber:"P-0037", name:"铝型材 4040-500mm", material:"6063铝合金",spec:"40×40mm",   remark:"",          prices:[{label:"标准价",value:38}],                                  createdAt:"2026-02-10" },
  { id:6, partNumber:"P-0036", name:"气缸 SC63×100",     material:"铝合金",   spec:"Φ63,行程100",remark:"标准型",   prices:[{label:"标准价",value:188}],                                 createdAt:"2026-02-05" },
]
...
function SortIcon({ sorted }: { sorted: false | "asc" | "desc" }) {
  if (sorted === "asc")  return <i className="ri-sort-asc text-xs text-foreground ml-1" />
  if (sorted === "desc") return <i className="ri-sort-desc text-xs text-foreground ml-1" />
  return <i className="ri-expand-up-down-line text-xs text-muted-foreground/40 ml-1" />
}
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex items-center gap-6 px-5 py-3.5 border-b border-border">
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-3.5 border-b border-border bg-background shrink-0 flex-wrap gap-y-2">
        <div>
          <h1 className="text-lg font-semibold tracking-tight leading-none">零件库</h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            {table.getFilteredRowModel().rows.length} / {MOCK.length} 个零件
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 h-8 px-2.5 border border-input rounded-md bg-background ml-4 flex-1 max-w-72">
          ...
          />
          {globalFilter && (
            <button onClick={() => setFilter("")} className="text-muted-foreground hover:text-foreground transition-colors bg-transparent border-none cursor-pointer p-0">
              ...
              <TableRow key={hg.id}>
                {hg.headers.map((h) => (
                  <TableHead
                    ...
                    className={h.column.getCanSort() ? "cursor-pointer select-none hover:bg-muted/80 whitespace-nowrap" : "whitespace-nowrap"}
                  >
            ...
            {table.getRowModel().rows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={columns.length} className="text-center py-16 text-muted-foreground">
                  ...
                  {globalFilter ? `没有匹配 "${globalFilter}" 的结果` : "暂无零件数据"}
                </TableCell>
              </TableRow>
            ) : (
      ...
      {/* Side panel sheet */}
      <Sheet open={panel !== null} onOpenChange={(o) => { if (!o) { setPanel(null); setEditing(null) } }}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>
              {panel === "add" ? (editingPart ? "编辑零件" : "新增零件") : "批量导入零件"}
            </SheetTitle>
            <SheetDescription>
              {panel === "add"
                ? "填写零件信息，系统自动生成零件编号"
          ...
          <div className="mt-6">
            {panel === "add" && (
              <PartForm
                ...
                onSubmit={handleAdd}
                onCancel={() => { setPanel(null); setEditing(null) }}
              />
            )}
            {panel === "import" && (
...
function MobileParts() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Mobile sheet */}
      <Sheet open={panel !== null} onOpenChange={(o) => { if (!o) setPanel(null) }}>
        <SheetContent side="bottom" className="h-[90vh] overflow-y-auto rounded-t-2xl">
          <SheetHeader>
            <SheetTitle>{panel === "add" ? "新增零件" : "批量导入零件"}</SheetTitle>
            <SheetDescription>
              {panel === "add" ? "填写零件信息，系统自动生成零件编号" : "上传 Excel 文件，自动解析并校验"}
            </SheetDescription>
          </SheetHeader>
          <div className="mt-6">
            {panel === "add" && (
              <PartForm onSubmit={handleAdd} onCancel={() => setPanel(null)} />
            )}
            {panel === "import" && (
              <ImportPanel onImport={handleImport} onClose={() => setPanel(null)} />
            )}
