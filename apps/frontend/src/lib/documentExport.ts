...
function expandColumnWidthsToPrintableWidth(
  ...
  const diff = Number(
    (targetTotalWidth - scaled.reduce((sum, width) => sum + width, 0)).toFixed(
      2,
    ),
  );

  if (scaled.length > 0 && diff !== 0) {
    scaled[scaled.length - 1] = Number(
      (scaled[scaled.length - 1] + diff).toFixed(2),
    );
  }
...
function ensureFitToA4SheetXml(
  ...
  const withFitFlag = (() => {
    ...
    if (xml.includes("<sheetPr")) {
      return xml.replace(
        /<sheetPr\b[^>]*>/,
        (match) => `${match}<pageSetUpPr fitToPage="1"/>`,
      );
    }
  ...
  const pageSetupTag = `<pageSetup paperSize="9" orientation="${orientation}" fitToWidth="1" fitToHeight="0"/>`;
  ...
  if (withPrintOptions.includes("<pageMargins")) {
    return withPrintOptions.replace(
      /(<pageMargins\b[^>]*\/>)/,
      `$1${pageSetupTag}`,
    );
  }
...
function buildWorkbook(
  ...
  const ws = buildWorksheet(rows, colWidths, contentStartRow, detailRowHeight);
  ...
  wb.Workbook.Names = [
    {
      ...
      Sheet: 0,
      Ref: `'${sheetName.replace(/'/g, "''")}'!$1:$${contentStartRow + 1}`,
    },
  ];
  return wb;
}
...
function appendSheetToWorkbook(wb: XLSX.WorkBook, payload: SheetPayload) {
  ...
  const ws = buildWorksheet(payload.rows, colWidths, payload.contentStartRow);
...
function buildWorksheet(
  ...
  ws["!rows"] = rows.map((row, index) => {
    ...
    return {
      hpt: Math.max(
        resolveRowHeight(row, colWidths),
        index > contentStartRow && index < rows.length - 1
          ? (detailRowHeight ?? 0)
          : 0,
      ),
    };
...
export function exportOrderPriceSheet(
  ...
  return writeWorkbookFitToA4(wb, {
    horizontallyCentered: payload.printHorizontallyCentered,
  }).then((bytes) => {
    downloadXlsx(payload.filename, bytes);
...
export function buildSheetPayloadXlsx(
  ...
  const wb = buildWorkbook(
    ...
    payload.contentStartRow,
    payload.printTargetWidthWch,
    payload.detailRowHeight,
  );
  return writeWorkbookFitToA4(wb, {
    horizontallyCentered: payload.printHorizontallyCentered,
    orientation: payload.printOrientation,
  });
}

async function exportSheetPayload(payload: SheetPayload): Promise<string> {
  const bytes = await buildSheetPayloadXlsx(payload);
  downloadXlsx(payload.filename, bytes);
  return payload.filename;
}

export function exportOrderDetailSheet(order: OrderDetail): Promise<string> {
  return exportSheetPayload(buildOrderDetailSheetPayload(order));
}

export function exportOrderDraftDetailSheet(
  draft: OrderDraftDetail,
): Promise<string> {
  return exportSheetPayload(buildOrderDraftDetailSheetPayload(draft));
}

export function exportMergedOrderDetailSheet(
  mergedOrder: MergedOrderDetail,
): Promise<string> {
  return exportSheetPayload(buildMergedOrderDetailSheetPayload(mergedOrder));
}
