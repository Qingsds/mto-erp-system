...
export const useUIStore = create<UIState>()(
  persist(
    (set, get) => ({
      ...
      toggleDark: ()  => { const v = !get().isDark; set({ isDark: v }); applyTheme(v) },

      setFontSize:   (v) => { set({ fontSize: v });   applyFont(v, get().lineHeight) },
      setLineHeight: (v) => { set({ lineHeight: v }); applyFont(get().fontSize, v) },
